-- Track all tables that have a phone_number column and check every column for non-null/non-empty values for phone 8989898989
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('SELECT COUNT(*) FROM %I WHERE phone_number = %L', tbl.table_name, '8989898989') INTO row_count;
    ...
    LOOP
      ...
      IF col.data_type ILIKE 'character%' OR col.data_type = 'text' THEN
        EXECUTE format('SELECT EXISTS(SELECT 1 FROM %I WHERE phone_number = %L AND %I IS NOT NULL AND trim(%I) <> '''')', tbl.table_name, '8989898989', col.column_name, col.column_name) INTO col_filled;
      ELSE
        EXECUTE format('SELECT EXISTS(SELECT 1 FROM %I WHERE phone_number = %L AND %I IS NOT NULL)', tbl.table_name, '8989898989', col.column_name) INTO col_filled;
      END IF;
    ...
    sql := format('SELECT %s as filled_columns FROM %I WHERE phone_number = %L', flags_expr, tbl.table_name, '8989898989');
