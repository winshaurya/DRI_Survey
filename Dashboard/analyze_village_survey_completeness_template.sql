-- ===========================================
-- VILLAGE SURVEY COMPLETENESS ANALYSIS — TEMPLATE
-- ===========================================
-- Template to analyze village survey completeness by `shine_code`.
-- Default target: SHINE_081 (change the value in the `params` CTE to run for another code)
--
-- Usage:
-- - Supabase SQL editor: Open the SQL editor, paste the file and run.
-- - psql / local Postgres: Save file and run `psql -h HOST -U USER -d DB -f analyze_village_survey_completeness_template.sql`
-- - Export to CSV (psql): e.g.
--   \copy (SELECT * FROM village_table_existence) TO 'village_table_existence_SHINE_081.csv' CSV HEADER;
--
-- How to change the target shine_code:
-- - Edit the `params` CTE below and set a different string (e.g. 'SHINE_001').
-- - All queries in this file read the value from `params` so no other edits are required.

-- -----------------------------
-- PARAMETERS (change here)
-- -----------------------------
-- Set the target shine_code here (default = 'SHINE_081')
-- Persist parameter in a temp table so it is available to all statements in this script
CREATE TEMP TABLE IF NOT EXISTS params (shine_code text);
TRUNCATE params;
INSERT INTO params VALUES ('SHINE_081');

-- First, let's check which tables have ANY data for the selected shine_code
WITH village_table_existence AS (
    SELECT
        'village_survey_sessions' as table_name,
        CASE WHEN EXISTS (SELECT 1 FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params)) THEN 1 ELSE 0 END as has_data
    UNION ALL
    SELECT 'village_population', CASE WHEN EXISTS (SELECT 1 FROM village_population WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_farm_families', CASE WHEN EXISTS (SELECT 1 FROM village_farm_families WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_housing', CASE WHEN EXISTS (SELECT 1 FROM village_housing WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_agricultural_implements', CASE WHEN EXISTS (SELECT 1 FROM village_agricultural_implements WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_crop_productivity', CASE WHEN EXISTS (SELECT 1 FROM village_crop_productivity WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_animals', CASE WHEN EXISTS (SELECT 1 FROM village_animals WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_irrigation_facilities', CASE WHEN EXISTS (SELECT 1 FROM village_irrigation_facilities WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_drinking_water', CASE WHEN EXISTS (SELECT 1 FROM village_drinking_water WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_transport', CASE WHEN EXISTS (SELECT 1 FROM village_transport WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_entertainment', CASE WHEN EXISTS (SELECT 1 FROM village_entertainment WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_medical_treatment', CASE WHEN EXISTS (SELECT 1 FROM village_medical_treatment WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_disputes', CASE WHEN EXISTS (SELECT 1 FROM village_disputes WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_educational_facilities', CASE WHEN EXISTS (SELECT 1 FROM village_educational_facilities WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_social_consciousness', CASE WHEN EXISTS (SELECT 1 FROM village_social_consciousness WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_children_data', CASE WHEN EXISTS (SELECT 1 FROM village_children_data WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_malnutrition_data', CASE WHEN EXISTS (SELECT 1 FROM village_malnutrition_data WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_bpl_families', CASE WHEN EXISTS (SELECT 1 FROM village_bpl_families WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_kitchen_gardens', CASE WHEN EXISTS (SELECT 1 FROM village_kitchen_gardens WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_seed_clubs', CASE WHEN EXISTS (SELECT 1 FROM village_seed_clubs WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_biodiversity_register', CASE WHEN EXISTS (SELECT 1 FROM village_biodiversity_register WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_traditional_occupations', CASE WHEN EXISTS (SELECT 1 FROM village_traditional_occupations WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_drainage_waste', CASE WHEN EXISTS (SELECT 1 FROM village_drainage_waste WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_signboards', CASE WHEN EXISTS (SELECT 1 FROM village_signboards WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_unemployment', CASE WHEN EXISTS (SELECT 1 FROM village_unemployment WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_social_maps', CASE WHEN EXISTS (SELECT 1 FROM village_social_maps WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_transport_facilities', CASE WHEN EXISTS (SELECT 1 FROM village_transport_facilities WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_infrastructure', CASE WHEN EXISTS (SELECT 1 FROM village_infrastructure WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_infrastructure_details', CASE WHEN EXISTS (SELECT 1 FROM village_infrastructure_details WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_survey_details', CASE WHEN EXISTS (SELECT 1 FROM village_survey_details WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_map_points', CASE WHEN EXISTS (SELECT 1 FROM village_map_points WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_forest_maps', CASE WHEN EXISTS (SELECT 1 FROM village_forest_maps WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
    UNION ALL
    SELECT 'village_cadastral_maps', CASE WHEN EXISTS (SELECT 1 FROM village_cadastral_maps WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params))) THEN 1 ELSE 0 END
)
...
-- Get the session_id for the selected shine_code
WITH session_data AS (
    SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params)
)
...
WHERE shine_code = (SELECT shine_code FROM params);
...
WHERE session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = (SELECT shine_code FROM params));
...
-- (remaining analyses unchanged — all reference the `params` CTE)
...
-- -----------------------------
-- COLUMN-FILL DETAILS (per-table column stats + samples)
-- -----------------------------
-- Output 1: columns that are completely unfilled for the selected shine_code
-- Output 2: filled columns with count and up to 10 sample non-null values

CREATE TEMP TABLE IF NOT EXISTS column_fill_stats (
  table_name text,
  column_name text,
  data_type text,
  filled_count bigint,
...
DECLARE
  ...
  v_samples text;
BEGIN
  SELECT shine_code INTO v_shine FROM params LIMIT 1;

  FOR t IN
    SELECT unnest(ARRAY[
      'village_survey_sessions','village_population','village_farm_families','village_housing',
      ...
      'village_educational_facilities','village_social_consciousness','village_children_data',
      'village_malnutrition_data','village_bpl_families','village_kitchen_gardens',
      'village_seed_clubs','village_biodiversity_register','village_traditional_occupations',
      'village_drainage_waste','village_signboards','village_unemployment',
      'village_social_maps','village_transport_facilities','village_infrastructure',
      'village_infrastructure_details','village_survey_details','village_map_points',
      'village_forest_maps','village_cadastral_maps'
    ])
  LOOP
    IF t = 'village_survey_sessions' THEN
      v_where := format('shine_code = %L', v_shine);
    ELSE
      v_where := format('session_id IN (SELECT session_id FROM village_survey_sessions WHERE shine_code = %L)', v_shine);
    END IF;
    ...
    LOOP
      IF c.data_type IN ('character varying','text','character') THEN
        EXECUTE format('SELECT COUNT(*) FROM %I WHERE %s AND %I IS NOT NULL AND %I <> ''''', t, v_where, c.column_name, c.column_name) INTO v_filled;
        EXECUTE format('SELECT array_to_string(array_agg(DISTINCT %I::text), '','') FROM (SELECT %I FROM %I WHERE %s AND %I IS NOT NULL AND %I <> '''' LIMIT 10) _', c.column_name, c.column_name, t, v_where, c.column_name, c.column_name) INTO v_samples;
      ELSE
        EXECUTE format('SELECT COUNT(*) FROM %I WHERE %s AND %I IS NOT NULL', t, v_where, c.column_name) INTO v_filled;
        EXECUTE format('SELECT array_to_string(array_agg(DISTINCT %I::text), '','') FROM (SELECT %I::text FROM %I WHERE %s AND %I IS NOT NULL LIMIT 10) _', c.column_name, c.column_name, t, v_where, c.column_name) INTO v_samples;
      END IF;

      INSERT INTO column_fill_stats (table_name, column_name, data_type, filled_count, sample_values)
      VALUES (t, c.column_name, c.data_type, COALESCE(v_filled,0), COALESCE(v_samples,''));
    END LOOP;
  END LOOP;
END$$ LANGUAGE plpgsql;

-- Columns that are completely unfilled
SELECT table_name, column_name
FROM column_fill_stats
WHERE filled_count = 0
ORDER BY table_name, column_name;

-- Filled columns with counts and up to 10 sample values
SELECT table_name, column_name, data_type, filled_count, COALESCE(sample_values,'(no sample)') AS sample_values
FROM column_fill_stats
WHERE filled_count > 0
ORDER BY table_name, column_name;

-- Clean up: drop intermediate temp table now that report is produced
DROP TABLE IF EXISTS column_fill_stats;
